...
int main()
{
    ...
    /* 250912 rectifier 노드에서 쓰는 값 그대로 */
    cv::Mat K = (cv::Mat_<double>(3, 3) << 
    ...
    cv::fisheye::estimateNewCameraMatrixForUndistortRectify(
        K, D, image_size_, cv::Mat::eye(3,3,CV_64F), K_rect, balance_
    );
    
    cv::Mat map1_, map2_;
    cv::fisheye::initUndistortRectifyMap(
    // cv::initUndistortRectifyMap(
        K, D, cv::Mat::eye(3, 3, CV_64F), K_rect, image_size_, CV_16SC2, map1_, map2_
    );
    /// solvePnP -> extrinsic
    // 3D 지면 좌표 (X: 전방+, Y: 좌측+)
    ...
    cv::fisheye::undistortPoints(p_2d, p_2d_undist, K, D, cv::noArray(), K_rect);
    ...
    bool success = cv::solvePnP(p_3d, p_2d_undist, K_rect, cv::noArray(), rvec, tvec, false, cv::SOLVEPNP_IPPE);
    ...
    CamCalib calib = CamCalib::Make(K_rect, D, R_w2c, t_w2c); // 모두 왜곡 보정된 거 기준으로
    ...
    cv::fisheye::undistortImage(image, undistorted_image, K, D, K_rect);
    ...
    // cv::projectPoints(guide, rvec, tvec, K_rect, D, p_proj);
    for (auto& g:guide) {
        p_proj.push_back(gndToPix(cv::Point2d(g.x, g.y),calib));
    }
    // cv::undistortPoints(p_proj, p_proj_undist, K, D, cv::noArray(), K);
    // cv::fisheye::undistortPoints(p_proj, p_proj_undist, K, D, cv::noArray(), K_rect);
    ...
    for (size_t i = 0; i < p_3d.size(); ++i) {
        ...
        try {
            ...
            cv::Point2d p_2dTo3d = pixToGndUndist(p_2d_undist[i], calib);
            std::cout << "Point " << i+1 << ": Pixel " << p_2d_undist[i]
                      << " -> World " << p_2dTo3d 
            ...
            cv::drawMarker(undistorted_image, p_proj[i], cv::Scalar(0, 255, 0), cv::MARKER_CROSS, 10, 2);
